...
//        ir  = null;
    ...
    public static boolean dbThreadCheck() { return Looper.myLooper().equals(db.getLooper()); }
    ...
    public final static Future<?> bgndPost(final String clazz, final String msg, final int priority, final Runnable run) {
        return bgnd.submit(new PriorityExecutor.Important() {
            ...
            public void run() {
                ...
                try { run.run(); }
                catch (Throwable ex) { Log.e(clazz, ex); }
                finally { Log.d(clazz, "BGND THREAD "+msg+" ("+(System.nanoTime()-startTime)/1000000+"ms)"); }
            }
        });
    }
    ...
    public final static Runnable nuiPost(final String clazz, final String msg, final Runnable run, final long delay) {
        final Runnable runner = new Runnable() {
            public void run() {
                ...
                finally {Log.d(clazz, "NON-UI THREAD "+(null==msg?"":msg)+" ("+(System.nanoTime()-startTime)/1000000+"ms)"); }
            }
        };
    ...
    public final static Runnable dbPost(final String clazz, final String msg, final Runnable run, final long delay) {
        final Runnable runner = new Runnable() {
            public void run() {
                ...
                finally {Log.d(clazz, "DB THREAD "+(null==msg?"":msg)+" ("+(System.nanoTime()-startTime)/1000000+"ms)"); }
            }
        };
    ...
    public final static Runnable uiPost(final String clazz, final String msg, final Runnable run, final long delay) {
        final Runnable runner = new Runnable() {
            public void run() {
                ...
                finally { Log.d(clazz, "UI THREAD "+(null==msg?"":msg)+" ("+(System.nanoTime()-startTime)/1000000+"ms)"); }
            }
        };
     ...
     *
     */
    public static abstract class OnComplete implements Runnable {
        ...
        public void run() { Log.e(OnComplete.class.getName(), "run not implemented!"); }
        ...
        private void execute() {
            ...
            final Runnable onComplete = new Runnable() {
                public void run() {
                    try { thread.run(); }
                    catch (Throwable ex) { Log.e("OnComplete", ex); }
                }
            };

            switch(mode) {
                default:
                   Log.e(ApplicationThread.class.getName(), new RuntimeException("UNRECOGNIZED OnComplete mode "+mode+" posting to NUI"));

                case OnComplete.NUI:
                	 Log.e("OnComplete NUI","NUI");
                    ApplicationThread.nuiPost(OnComplete.class.getName(), "OnComplete", onComplete);
                    break;

                case OnComplete.UI:
               	 Log.e("OnComplete UI","UI");

                    ApplicationThread.uiPost(OnComplete.class.getName(), "OnComplete", onComplete);
                    break;

                case OnComplete.BGND:
               	Log.e("OnComplete BG","BG");
    ...
    public static abstract class Observable {
        ...
        public final void add(final Object observer) {
            synchronized(observers) {
                ...
                if (observers.contains(observer)) { Log.e(this.getClass().getName(),"ERROR already contains class:"+observer.getClass().getName()+android.util.Log.getStackTraceString(new Throwable())); return; }
        ...
        public final void remove(final Object observer) {
            synchronized(observers) {
                if (!observers.contains(observer)) { Log.e(this.getClass().getName(),"ERROR does not contain class:"+observer.getClass().getName()+android.util.Log.getStackTraceString(new Throwable())); return; }
        ...
        public static abstract class Message {
            public void event(final int id, final Object obj, final Object... param) { Log.v(getClass().getName(),"event(int,object,object) not implemented"); }
        }

        public final void notify(final int id,final Object obj,final Object... param) {
           ApplicationThread.nuiPost(getClass().getName(), "notify observers", new Runnable() {
                public void run() {
